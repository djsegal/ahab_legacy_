...
def readReal( curMode , inpValues ) :
    
    if curMode > 1:
        if inpValues[2] != round(inpValues[2]):
            print('note: numVals was rounded to an integer value.')
        inpValues[2] = int(round(inpValues[2]))
        if  inpValues[2] < 1:
            print('note: numVals was less than zero. it was changed to zero.)')
            inpValues[2] = 1
    ...
    if curMode == 1:            # init : increment : final
        ...
        if inpValues[0] != inpValues[2] and inpValues[1] != 0:
            done = False
            while ( not done ):
                wrkValue = curValues[-1] + inpValues[1]
                # make sure the last value is inpValues[2]
                if abs(wrkValue) + abs(inpValues[1])/2 > abs(inpValues[2]):
